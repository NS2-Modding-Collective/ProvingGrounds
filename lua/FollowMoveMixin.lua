...
FollowMoveMixin.expectedConstants =
{
}

FollowMoveMixin.defaultConstants =
{
}

function FollowMoveMixin.__initmixin(classInstance)
    classInstance.followedTargetId = nil
    classInstance.imposedTargetId  = nil
end
...
local function ChangeTarget(self, reverse)
    ...
    if nextTargetIndex <= numberOfTargets then
        ...
        self:SetDesiredCamera(0.0, { move = true}, targets[nextTargetIndex]:GetOrigin(), nil, 5)
...
local function UpdateView(self, input)
    ...
    local targetId      = self.imposedTargetId ~= 0 and self.imposedTargetId or self.followedTargetId
 ...
 * Move the player with an overhead point of view
 */
function FollowMoveMixin:ImposeTarget(target)

    if target then
        self.imposedTargetId = target:GetId()
    else
        self.imposedTargetId = 0
    end

end
AddFunctionContract(FollowMoveMixin.UpdateMove, { Arguments = { "Entity", "Move" }, Returns = { } })