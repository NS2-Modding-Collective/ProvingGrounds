...
local kRange = 1.4
Builder.kBuildEffectInterval = 0.2
...
function Builder:ProcessMoveOnWeapon(player, input)
    ...
    elseif Client and self.playEffect then

        if self.lastBuilderEffect + Builder.kBuildEffectInterval <= Shared.GetTime() then
        
            self:TriggerEffects("builder_construct")
