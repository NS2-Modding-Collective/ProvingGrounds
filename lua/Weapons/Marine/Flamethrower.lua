...
// TODO: remove once cinematics are positioned 1 frame earlier than currently
local lastCameraAngles = Angles(0,0,0)
...
if Client then
    ...
    function Flamethrower:InitTrailCinematic()
        ...
        if self:GetParent() == Client.GetLocalPlayer() then
            ...
            self.trailCinematic:AttachToFunc(self, TRAIL_ALIGN_Z, Vector(-0.07, -0.1, 0.9),
                function (attachedEntity, deltaTime)
                
                    local player = Client.GetLocalPlayer()
                    local position = player:GetEyePos()
                    position = position + (position - lastPosition) * 0.8
                    
                    local cameraAngles = player:GetViewAngles()
                    local angleDiff = Angles(GetAnglesDifference(cameraAngles.pitch, lastCameraAngles.pitch ) * 0.2, GetAnglesDifference(cameraAngles.yaw, lastCameraAngles.yaw) * 0.2, 0)
                    
                    local extrapolatedAngles = Angles(cameraAngles)
                    extrapolatedAngles.pitch = cameraAngles.pitch - angleDiff.pitch
                    extrapolatedAngles.yaw = cameraAngles.yaw - angleDiff.yaw
                    
                    local coords = extrapolatedAngles:GetCoords()
                    coords.origin = position
                    
                    // save for next frame
                    lastCameraAngles = cameraAngles
                    lastPosition = player:GetEyePos()
    ...
    function Flamethrower:CreateImpactEffect(player)
        ...
        if not self.timeLastImpactEffect or self.timeLastImpactEffect + kImpactEffectRate < Shared.GetTime() then
