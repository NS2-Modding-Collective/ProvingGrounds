...
LayMines.kModelName = PrecacheAsset("models/marine/mine/mine_pile.model")
...
function LayMines:OnInitialized()
    ...
    self:SetModel(LayMines.kModelName)

end

function LayMines:GetIsValidRecipient(recipient)

    if recipient and not GetIsVortexed(recipient) and recipient:isa("Marine") then
...
function LayMines:GetPositionForStructure(player)
    ...
    local origin = player:GetEyePos() + player:GetViewAngles():GetCoords().zAxis * LayMines.kPlacementDistance
    ...
    if trace.fraction == 1 then
        ...
        origin = player:GetEyePos() + player:GetViewAngles():GetCoords().zAxis * LayMines.kPlacementDistance
        trace = Shared.TraceRay(origin, origin - Vector(0, LayMines.kPlacementDistance, 0), CollisionRep.Default, PhysicsMask.AllButPCsAndRagdolls, EntityFilterTwo(player, self))
...
function LayMines:ProcessMoveOnWeapon(input)
...
function LayMines:OverrideWeaponName()
    return "mine"
end

function LayMines:OnGetIsVisible(visTable)
