...
Script.Load("lua/Weapons/Marine/BMFGWeaponSlotMixin.lua")
...
AddMixinNetworkVars(BMFGWeaponSlotMixin, networkVars)
...
function Minigun:OnCreate()
    ...
    InitMixin(self, BMFGWeaponSlotMixin)
...
function Minigun:OnUpdateRender()
    ...
    if parent and parent:GetIsLocalPlayer() then
        ...
        if viewModel and viewModel:GetRenderModel() then
            ...
            viewModel:GetRenderModel():SetMaterialParameter("heatAmount" .. self:GetBMFGWeaponSlotName(), self.heatAmount)
...
function Minigun:OnUpdateAnimationInput(modelMixin)
    ...
    modelMixin:SetAnimationInput("activity_" .. self:GetBMFGWeaponSlotName(), activity)
...
if Client then
    ...
    local kAttachPoints = { [BMFG.kSlotNames.Left] = "fxnode_l_minigun_muzzle", [BMFG.kSlotNames.Right] = "fxnode_r_minigun_muzzle" }
    ...
    function Minigun:OnClientPrimaryAttacking()
        ...
        if parent then        
            CreateMuzzleCinematic(self, kMuzzleEffectName, kMuzzleEffectName, kAttachPoints[self:GetBMFGWeaponSlot()] , parent)        
        end
