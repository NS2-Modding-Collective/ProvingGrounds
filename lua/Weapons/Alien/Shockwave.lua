...
function Shockwave:OnCreate()
    ...
    if Server then
        ...
        self:AddTimedCallback(Shockwave.UpdateDisruptables, kUpdateRate)
...
function Shockwave:UpdateDisruptables()

    for index, ent in ipairs(GetEntitiesWithMixinWithinRange("Disruptable", self:GetOrigin(), 3)) do
        ent:SetDisruptDuration(kDisruptTime)
    end
