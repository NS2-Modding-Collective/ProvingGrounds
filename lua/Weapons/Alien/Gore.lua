...
function Gore:OnCreate()
    ...
    self.lastGoreSmashdEntityId = Entity.invalidId
...
function Gore:GetEffectParams(tableParams)
    ...
    if self.lastGoreSmashdEntityId ~= Entity.invalidId then
    
        local lastGoreSmashdEntity = Shared.GetEntity(self.lastGoreSmashdEntityId)
        if lastGoreSmashdEntity and GetReceivesStructuralDamage(lastGoreSmashdEntity) then
