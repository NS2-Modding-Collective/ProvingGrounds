...
function GUIMinimapButtons:OnResolutionChanged()    
end

function GUIMinimapButtons:SetMinimapScript(minimapScript)

    self.minimapScript = minimapScript
    minimapScript:GetBackground():AddChild(self.background)
...
function GUIMinimapButtons:Update(deltaTime)
    ...
    if self.minimapScript then
        ...
        //local texCoords = ConditionalValue(self.minimapScript:GetPingEnabled(), kPingButtonActiveTexCoords, kPingButtonTexCoords)
        //self.pingButton:SetTexturePixelCoordinates(unpack(texCoords))
        
        local alpha = 0.7
        if self.minimapScript:GetPingEnabled() then
            alpha = 1
        end
...
function GUIMinimapButtons:SendKeyEvent(key, down)
    ...
    if key == InputKey.MouseButton1 then
            self.minimapScript:SetPingEnabled(false)
    end
    ...
    if self.minimapScript then
        ...
        if key == InputKey.MouseButton0 and CommanderUI_GetUIClickable() then
            ...
            if GUIItemContainsPoint(self.pingButton, mouseX, mouseY) then
                ...
                self.minimapScript:SetPingEnabled(true)
                return true
