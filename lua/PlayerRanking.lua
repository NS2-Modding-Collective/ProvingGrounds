            ...
            if client and not client:GetIsVirtual() then
                ...
                if not self.capturedPlayerData[steamId..""] then
                    ...
                    local playerData = 
                    {
                        ...
                        greenTime = player:GetGreenPlayTime(),
                        purpleTime = player:GetPurplePlayTime(),
                    ...
                    playerData.greenTime = player:GetGreenPlayTime()
                    playerData.purpleTime = player:GetPurplePlayTime()
...
function PlayerRanking:InsertPlayerData(playerTable, recordedData, winningTeam, gameTime)
    ...
    if GetPlayerIsValidForRanking(recordedData, gameTime) then
        ...
        local playerData = 
        {
            ...
            greenTime = recordedData.greenTime,
            purpleTime = recordedData.purpleTime,
...
function PlayerRanking:GetAveragePlayerSkill()
    ...
    if not self.timeLastSkillUpdate or self.timeLastSkillUpdate < Shared.GetTime() then
        ...
        self.averagePlayerSKills = { 
            [kGreenTeamType] = { numPlayers = 0, skillSum = 0 }, 
            [kPurpleTeamType] = { numPlayers = 0, skillSum = 0 },
        ...
        for _, player in ipairs(GetEntitiesWithMixin("Scoring")) do
            ...
            if client and skill then // and not client:GetIsVirtual()
                ...
                if teamType == kGreenTeamType or teamType == kPurpleTeamType then
        ...
        self.averagePlayerSKills[kGreenTeamType].averageSkill = self.averagePlayerSKills[kGreenTeamType].numPlayers == 0 and 0 or self.averagePlayerSKills[kGreenTeamType].skillSum / self.averagePlayerSKills[kGreenTeamType].numPlayers
        self.averagePlayerSKills[kPurpleTeamType].averageSkill = self.averagePlayerSKills[kPurpleTeamType].numPlayers == 0 and 0 or self.averagePlayerSKills[kPurpleTeamType].skillSum / self.averagePlayerSKills[kPurpleTeamType].numPlayers
    ...
    return self.averagePlayerSKills[3].averageSkill, self.averagePlayerSKills[kGreenTeamType].averageSkill, self.averagePlayerSKills[kPurpleTeamType].averageSkill
