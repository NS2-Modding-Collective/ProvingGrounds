...
Crag.kHealWaveEffectInterval = 0.4
...
function Crag:OnUpdate(deltaTime)
    ...
    elseif Client then
        ...
        if self.healWaveActive then
            ...
            if not self.lastHealWaveEffect or self.lastHealWaveEffect + Crag.kHealWaveEffectInterval < Shared.GetTime() then
        
                self:TriggerEffects("crag_heal_wave")
                self.lastHealWaveEffect = Shared.GetTime()
...
function Crag:GetTechButtons(techId)
    ...
    local techButtons = { kTechId.SetRally, kTechId.CragHeal, kTechId.None, kTechId.None, 
                    kTechId.HealWave, kTechId.None, kTechId.None, kTechId.CragBabblers }
