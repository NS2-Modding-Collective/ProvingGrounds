...
function Hydra:OnInitialized()
    ...
    if Server then
        ...
        // gorges can use the hydra and diggest it
        self.diggestAmount = 0.0
...
function Hydra:GetCanDiggest(player)
    return  player:GetIsAlive() and self:GetIsBuilt() and (player:GetId() == self.hydraParentId and self.hydraParentId ~= Entity.invalidId) and
            player:isa("Gorge") and (not HasMixin(self, "Live") or self:GetIsAlive())
end

function Hydra:GetCanBeUsed(player, useSuccessTable)
    useSuccessTable.useSuccess = useSuccessTable.useSuccess and self:GetCanDiggest(player)    
end
