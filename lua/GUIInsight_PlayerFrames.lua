...
local kGreenTexture = "ui/marine_sheet.dds"
local kPurpleTexture = "ui/alien_sheet.dds"
...
function GUIInsight_PlayerFrames:Initialize()
    ...
    self.teams = {
        ...
        [kTeam1Index] = {
            ...
            Color         = kGreenColor,
            PlayerList    = {},
            GetScores     = ScoreboardUI_GetGreenScores,
        ...
        [kTeam2Index] = {
            ...
            Color         = kPurpleColor,
            PlayerList    = {},
            GetScores     = ScoreboardUI_GetPurpleScores,
...
function GUIInsight_PlayerFrames:CreateBackground(teamNumber)
    ...
    if teamNumber == kTeam1Index then
        ...
        playersBackground:SetColor(kGreenColor)
    ...
    elseif teamNumber == kTeam2Index then
        ...
        playersBackground:SetColor(kPurpleColor)
...
function GUIInsight_PlayerFrames:UpdatePlayer(player, playerRecord, team, yPosition)
    ...
    elseif isCommander then
        ...
        if playerRecord.Health then           
            local health = playerRecord.Health 
            local maxHealth = playerRecord.MaxHealth 
            local healthFraction = health/maxHealth
            
            barSize = math.max(healthFraction * kHealthBarSize.y, 0)
            local barColor
            if healthFraction > 0.6 then
                local purpleFraction = 1 - ((healthFraction - 0.6) / 0.4)
                barColor = Color(purpleFraction, 1, 0, 1)
