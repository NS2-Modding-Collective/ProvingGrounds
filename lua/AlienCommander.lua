...
if Server then
    ...
    function AlienCommander:TriggerMist(position, entity)
        ...
        if cost <= team:GetTeamResources() and HasMixin(entity, "Catalyst") then

            if entity:GetCanCatalyst() then
            
                entity:TriggerCatalyst(kNutrientMistDuration)
                success = true
                // create custom sound for alien commander
                StartSoundEffectForPlayer(Hive.kTriggerCatalyst2DSound, self)
                self:TriggerEffects("hive_nutrient_mist", {effecthostcoords = Coords.GetTranslation(position)})
...
if Server then
    ...
    function AlienCommander:ProcessTechTreeActionForEntity(techNode, position, normal, pickVec, orientation, entity, trace)
        ...
        elseif techId == kTechId.NutrientMist then
            ...
            success = self:TriggerMist(position, trace.entity)
            keepProcessing = false
...
function AlienCommander:GetQuickMenuTechButtons(techId)
    ...
    if techId == kTechId.BuildMenu then
        ...
        menuButtons = { kTechId.Cyst, kTechId.Harvester, kTechId.Whip, kTechId.Hive,
                        kTechId.None, kTechId.None, kTechId.None, kTechId.None }
        
    elseif techId == kTechId.AdvancedMenu then
    
        menuButtons = { kTechId.Crag, kTechId.Shade, kTechId.Shift, kTechId.None,
                        kTechId.Shell, kTechId.Veil, kTechId.Spur, kTechId.None }
        
    elseif techId == kTechId.AssistMenu then
    
        menuButtons = { kTechId.ThreatMarker, kTechId.NeedHealingMarker, kTechId.WeakMarker, kTechId.ExpandingMarker,
                        kTechId.NutrientMist, kTechId.InfestationSpike, kTechId.None, kTechId.None }
    ...
    return alienTechButtons, inQuickAccessMenu
