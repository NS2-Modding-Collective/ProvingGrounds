...
local kSheet1 = "ui/exosuit_HUD1.dds"
local kSheet2 = "ui/exosuit_HUD2.dds"
local kSheet3 = "ui/exosuit_HUD3.dds"
local kSheet4 = "ui/exosuit_HUD4.dds"
...
function GUIExoHUD:Initialize()
    ...
    local staticRing = GUIManager:CreateGraphicItem()
    staticRing:SetTexture(kSheet4)
    staticRing:SetTexturePixelCoordinates(unpack(kStaticRingCoords))
    local size = CoordsToSize(kStaticRingCoords)
    staticRing:SetSize(size)
    staticRing:SetAnchor(GUIItem.Middle, GUIItem.Center)
    staticRing:SetPosition(Vector(-size.x / 2, -size.y / 2, 0))
    staticRing:SetLayer(kGUILayerPlayerHUDForeground1)
    self.background:AddChild(staticRing)
