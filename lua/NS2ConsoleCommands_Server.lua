...
local function OnCommandDualMinigun(client)
    ...
    if Shared.GetCheatsEnabled() then
        ...
        if not player:isa("Exo") then
            player = player:Replace(Exo.kMapName, player:GetTeamNumber(), false)
        end
...
local function OnCommandDisrupt(client)
    ...
    if Shared.GetCheatsEnabled() and player and player.SetKnockback then
...
Event.Hook("Console_exo", OnCommandChangeClass("exo", kTeam1Index))
Event.Hook("Console_dualminigun", OnCommandDualMinigun )
