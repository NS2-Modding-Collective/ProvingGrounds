...
function Whip:GetTechButtons(techId)
    ...
    if self:GetIsUnblocked() then
            
       if(techId == kTechId.RootMenu) then 
            
            techButtons = { kTechId.None, kTechId.None, kTechId.Attack, kTechId.Cancel }

            local rootUnroot = self.rooted and kTechId.WhipUnroot or kTechId.WhipRoot
            table.insert(techButtons, rootUnroot)
            
            if not self:GetHasUpgrade(kTechId.WhipBombard) then
                techButtons[1] = kTechId.EvolveBombard
            else
...
function Whip:GetTechAllowed(techId, techNode, player)
    ...
    if techId == kTechId.Cancel then
        allowed = self:GetCurrentOrder() ~= nil or self:GetIsResearching()
    end
