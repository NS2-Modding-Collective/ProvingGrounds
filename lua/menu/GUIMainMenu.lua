    ...
    local hostOptions = 
        {
            {   
                name   = "ServerName",            
                label  = "SERVER NAME",
                value  = Client.GetOptionString("serverName", "PG Listen Server")
            },
            {   
                name   = "Password",            
...
local function InitOptions(optionElements)
    ...
    local avatarVariant = Client.GetOptionInteger("avatarVariant", -1)
    local skulkVariant = Client.GetOptionInteger("skulkVariant", -1)
    
    // if not set explicitly, always use the highest available tier
    if avatarVariant == -1 then

        for variant = 1,GetEnumCount(kAvatarVariant) do
            if GetHasVariant( kAvatarVariantData, variant ) then
    ...
    assert( avatarVariant ~= -1 )

    Client.SetOptionInteger("avatarVariant", avatarVariant)
    ...
    optionElements.AvatarVariantName:SetValue(GetVariantName(kAvatarVariantData,avatarVariant))
...
local function SendPlayerVariantUpdate(avatarVariant, sexType)

    assert( avatarVariant ~= -1 )
    assert( avatarVariant ~= nil )
...
local function SaveOptions(mainMenu)
    ...
    local avatarVariantName     = mainMenu.optionElements.AvatarVariantName:GetValue()
    ...
    Client.SetOptionInteger("avatarVariant", FindVariant( kAvatarVariantData, avatarVariantName ) )
    Client.SetOptionString("sexType", sexType)
    
    SendPlayerVariantUpdate(
            FindVariant( kAvatarVariantData, avatarVariantName ),
            sexType )
...
function GUIMainMenu:CreateOptionWindow()
    ...
    local avatarVariantNames = {}

    //DebugPrint("we have "..GetEnumCount(kAvatarVariant).." avatar variants")

    for key,value in pairs(kAvatarVariantData) do
        if GetHasVariant( kAvatarVariantData, key ) then
            table.insert( avatarVariantNames, value.displayName )
        end
    end
    ...
    local generalOptions =
        {
            ...
            {
                name    = "AvatarVariantName",
                label   = "AVATAR ARMOR",
                type    = "select",
                values  = avatarVariantNames
            },
            {
                name    = "SexType",
                label   = "MARINE GENDER",
...
function GUIMainMenu:MaybeCreateFirstRunWindow()
    ...
    okButton:AddEventCallbacks({ OnClick = function()
            ...
            Shared.ConsoleCommand("map pg_dm6")
        end})
