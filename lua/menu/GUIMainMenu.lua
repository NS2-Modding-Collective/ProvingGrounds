...
function GUIMainMenu:Initialize()
    ...
    if MainMenu_IsInGame() then
        ...
        self.disconnectLink = self:CreateMainLink("DISCONNECT", "disconnect", "05")
...
function GUIMainMenu:CreateExploreWindow()
    ...
    self.explore:AddEventCallbacks({
             OnShow = function (self)
                local mapNames
                mapNames = MainMenu_GetMapNameList()
...
local function InitOptions(optionElements)
    ...
    local shadowFading          = OptionsDialogUI_GetShadowFading()
    ...
    optionElements.ShadowFading:SetOptionActive( BoolToIndex(shadowFading) )
...
local function SaveOptions(mainMenu)
    ...
    local shadowFading          = mainMenu.optionElements.ShadowFading:GetActiveOptionIndex() > 1
