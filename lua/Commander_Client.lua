...
function Commander:OnDestroy()
    ...
    if self.hudSetup == true then
        ...
        DynamicMesh_Destroy(self.powerPackLineHelp)
...
function Commander:UpdatePowerPackLine(fromPoint)

    local consumersNearby = GetEntitiesWithMixinForTeamWithinRange("PowerConsumer", self:GetTeamNumber(), fromPoint, PowerPack.kRange)
    Shared.SortEntitiesByDistance(fromPoint, consumersNearby)
    
    local closestConsumer = consumersNearby[1]
    
    local inRange = false
    
    if closestConsumer then
        local distance = GetPathDistance(fromPoint, closestConsumer:GetOrigin())
        if distance and distance <= PowerPack.kRange then
            inRange = true
        end
    end    
    ...
    if inRange then
        ...
        local endPoint = closestConsumer:GetOrigin() + Vector(0, kZFightingConstant, 0)
        local direction = GetNormalizedVector(endPoint - startPoint)

        UpdateOrderLine(startPoint, endPoint, self.powerPackLineHelp)
        
        //DebugLine(startPoint, endPoint, 0.06, 0, 0, 1, 1)
        
    else
        self.powerPackLineHelp:SetIsVisible(false)
    end
...
function Commander:UpdateGhostStructureVisuals()
    ...
    self.powerPackLineHelp:SetIsVisible(self.currentTechId == kTechId.PowerPack)
    ...
    if self:GetShowGhostModel() then
        ...
        if self.currentTechId == kTechId.PowerPack then
            self:UpdatePowerPackLine(coords.origin)
        end 
...
function Commander:SetupHud()
    ...
    self.powerPackLineHelp = DynamicMesh_Create()
    self.powerPackLineHelp:SetIsVisible(false)
    self.powerPackLineHelp:SetMaterial(Commander.kMarineLineMaterialName)
...
local function UpdateCooldown(self, techId)
