...
// Sort players by last time resupplied so players aren't starved out when Armory low on energy
...
function Armory:ResupplyPlayers()
    ...
    for index, player in ipairs(self:GetSortedResupplyPlayerList()) do
        ...
        if self:GetShouldResupplyPlayer(player) then
            ...
            if self.resupplyEnergy >= Armory.kArmoryResupplyEnergy then
        
                self:ResupplyPlayer(player)                
                
                self.resupplyEnergy = (self.resupplyEnergy - Armory.kArmoryResupplyEnergy)
...
function Armory:OnThink()
    ...
    self.resupplyEnergy = math.min(self.resupplyEnergy + Armory.kArmoryResupplyEnergyRate * Armory.kThinkTime, Armory.kMaxArmoryResupplyEnergy)
