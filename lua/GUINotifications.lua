...
GUINotifications.kGreenColor = Color(205/255, 245/255, 1, 1)
GUINotifications.kTooltipGreenYOffset = -60

GUINotifications.kIconSize = Vector(64, 80, 0)
GUINotifications.kIconBigSize = Vector(96, 120, 0)

GUINotifications.kGreenTexture = PrecacheAsset("ui/marine_tooltips.dds")
GUINotifications.kGreenIconPixelCoords = { 0, 0, 64, 80 }

GUINotifications.kGeneralFont = "fonts/Arial_15.fnt"
GUINotifications.kGreenFont = "fonts/AgencyFB_small.fnt"

local kPurpleFont = "fonts/AgencyFB_small.fnt"
...
function GUINotifications:EnableGreenStyle()

    PROFILE("GUINotifications:EnableGreenStyle")

    self.locationText:SetColor(GUINotifications.kGreenColor)
    self.locationText:SetFontName(GUINotifications.kGreenFont)
    self.tooltipBackground:SetColor(Color(1,1,1,0))
    self.tooltipText:SetColor(GUINotifications.kGreenColor)
    self.scoreDisplay:SetColor(GUINotifications.kGreenColor)
    self.tooltipBackground:SetAnchor(GUIItem.Middle, GUIItem.Bottom)
    local currentWidth = self.tooltipBackground:GetSize().x
    self.tooltipBackground:SetPosition(Vector(-currentWidth / 2, GUINotifications.kTooltipGreenYOffset, 0))
    
    self.toolTipIcon:SetTexture(GUINotifications.kGreenTexture)
    self.toolTipIcon:SetTexturePixelCoordinates(unpack(GUINotifications.kGreenIconPixelCoords))
    
    self.toolTipIcon:SetIsVisible(true)
    
end

function GUINotifications:EnablePurpleStyle()

    PROFILE("GUINotifications:EnablePurpleStyle")

    self.locationText:SetColor(kPurpleTeamColorFloat)
    self.locationText:SetFontName(kPurpleFont)
    self.tooltipBackground:SetColor(GUINotifications.kTooltipBackgroundColor)
    self.tooltipText:SetColor(kPurpleTeamColorFloat)
...
function GUINotifications:Update(deltaTime)
    ...
    if PlayerUI_IsOnGreenTeam() then
        self:EnableGreenStyle()
    elseif PlayerUI_IsOnPurpleTeam() then
        self:EnablePurpleStyle()
    else
