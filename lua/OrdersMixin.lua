...
local function OrderTargetInvalid(self, targetId)
    ...
    if targetId and targetId ~= Entity.invalidId  then
        ...
        invalid = target and ( (target:isa("PowerPoint") and target:GetPowerState() == PowerPoint.kPowerState.unsocketed) or target:isa("ResourcePoint") or target:isa("TechPoint") 
