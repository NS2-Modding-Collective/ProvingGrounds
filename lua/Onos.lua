...
Onos.kDizzyAcceleration = 8
Onos.kBaseAcceleration = 16.4
Onos.kChargeAcceleration = 50

Onos.kDizzyDuration = 0.5
Onos.kDizzyDamageMultiplier = 1
// onos will be slower for kDizzyDuration when stopping at a speed higher than kChargeDizzySpeedThreshold
Onos.kChargeDizzySpeedThreshold = 8.5
...
function Onos:GetAcceleration()
    ...
    if self:GetIsDizzy() then
    
        acceleration = Onos.kDizzyAcceleration
...
function Onos:GetIsDizzy()
    return self.dizzyTime + Onos.kDizzyDuration > Shared.GetTime()
end
...
function Onos:EndCharge()
    ...
    if self.chargeSpeed > Onos.kChargeDizzySpeedThreshold then
        TriggerMomentumChangeEffects(self, surface, moveDirection, normal)
        self.dizzyTime = Shared.GetTime()
    end
...
function Onos:TriggerCharge(move)
    ...
    if not self:GetIsDizzy() and not self.charging and self.timeLastChargeEnd + Onos.kChargeDelay < Shared.GetTime() and self:GetIsOnGround() and not self:GetCrouching() then
...
function Onos:ComputeDamageOverride(attacker, damage, damageType, time)

    damage = Alien.ComputeDamageOverride(self, attacker, damage, damageType, time)
    
    if self:GetIsDizzy() then
        damage = damage * Onos.kDizzyDamageMultiplier
    end    
