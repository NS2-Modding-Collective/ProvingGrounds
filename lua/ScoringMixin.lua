...
function ScoringMixin:AddScore(points)
...
if Server then
    ...
    function ScoringMixin:GetGreenPlayTime()
        return self.greenTime
    end
    
    function ScoringMixin:GetPurplePlayTime()
        return self.purpleTime
    end
    ...
    function SharedUpdate(self, deltaTime)
        ...
        if not self.greenTime then
            self.greenTime = 0
        end
        
        if not self.purpleTime then
            self.purpleTime = 0
        end    
        ...
        if self:GetIsPlaying() then
            ...
            if self:GetTeamType() == kGreenTeamType then
                self.greenTime = self.greenTime + deltaTime
            end
            
            if self:GetTeamType() == kPurpleTeamType then
                self.purpleTime = self.purpleTime + deltaTime
            end
...
function ScoringMixin:AddContinuousScore(name, addAmount, amountNeededToScore, pointsGivenOnScore)
    ...
    if Server then
        ...
        while self.continuousScores[name].amount >= amountNeededToScore do
            ...
            self:AddScore(pointsGivenOnScore)
