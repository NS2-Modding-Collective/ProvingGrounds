...
local function GetEchoButtons()
    ...
    return {
        ...
        kTechId.TeleportShell, kTechId.TeleportVeil, kTechId.TeleportSpur, kTechId.TeleportEgg
...
function Shift:GetTechButtons(techId)
    ...
    if techId == kTechId.RootMenu then 
        ...
        techButtons = { kTechId.ShiftEnergize, kTechId.ShiftHatch, kTechId.ShiftEcho }

        if not self:GetHasUpgrade(kTechId.ShiftEcho) then
            techButtons[3] = kTechId.EvolveEcho
        end
...
if Server then
    ...
    function Shift:TriggerEcho(techId, position)
        ...
        local validPos = GetIsBuildLegal(techId, position, kStructureSnapRadius, self:GetOwner(), self)
...
function Shift:OverrideBuildEntity(techId, position, commander)

    local newEnt = nil
    
    if techId == kTechId.ShiftHatch then

        newEnt = CreateEntity(Egg.kMapName, position + Vector(0, 0.15, 0), self:GetTeamNumber())
        newEnt:SetOwner(commander)
        self:RegisterEgg(newEnt)
