...
GUICountDownDisplay.kGreenIconTexCoords = { 0, 64, 128, 128}
GUICountDownDisplay.kPurpleIconTexCoords = { 0, 704, 128, 768}
...
GUICountDownDisplay.kGreenLogoTexture = "ui/marine_logo_small.dds"
GUICountDownDisplay.kGreenLogoTexCoords = { 0, 0, 64, 96 }

GUICountDownDisplay.kPurpleLogoTexture = "ui/alien_portraiticons.dds"
GUICountDownDisplay.kPurpleLogoTexCoords = { 3, 164, 80, 240 }
...
function GUICountDownDisplay:Initialize()
    ...
    self.greenNames = GetGUIManager():CreateTextItem()
    self.greenNames:SetAnchor(GUIItem.Left, GUIItem.Top)
    self.greenNames:SetTextAlignmentX(GUIItem.Align_Min)
    self.greenNames:SetTextAlignmentY(GUIItem.Align_Center)
    self.greenNames:SetFontName(GUICountDownDisplay.kFontName)
    self.greenNames:SetFontSize(GUICountDownDisplay.kNameFontSize)
    self.nameFrame:AddChild(self.greenNames)
    
    self.purpleNames = GetGUIManager():CreateTextItem()
    self.purpleNames:SetAnchor(GUIItem.Left, GUIItem.Bottom)
    self.purpleNames:SetTextAlignmentX(GUIItem.Align_Min)
    self.purpleNames:SetTextAlignmentY(GUIItem.Align_Center)
    self.purpleNames:SetFontName(GUICountDownDisplay.kFontName)
    self.purpleNames:SetFontSize(GUICountDownDisplay.kNameFontSize)
    self.nameFrame:AddChild(self.purpleNames)
    
    self.greenIcon = GetGUIManager():CreateGraphicItem()
    self.greenIcon:SetAnchor(GUIItem.Left, GUIItem.Top)
    self.greenIcon:SetSize(GUICountDownDisplay.kIconSize)
    self.greenIcon:SetPosition( Vector( -GUICountDownDisplay.kIconSize.x - 10,  -GUICountDownDisplay.kIconSize.y / 2, 0) )
    self.greenIcon:SetTexture(GUICountDownDisplay.kIconTexture)
    self.greenIcon:SetTexturePixelCoordinates(unpack(GUICountDownDisplay.kGreenIconTexCoords))
    self.nameFrame:AddChild(self.greenIcon)
    
    self.purpleIcon = GetGUIManager():CreateGraphicItem()
    self.purpleIcon:SetAnchor(GUIItem.Left, GUIItem.Bottom)
    self.purpleIcon:SetSize(GUICountDownDisplay.kIconSize)
    self.purpleIcon:SetPosition( Vector( -GUICountDownDisplay.kIconSize.x - 10,  -GUICountDownDisplay.kIconSize.y / 2, 0) )
    self.purpleIcon:SetTexture(GUICountDownDisplay.kIconTexture)
    self.purpleIcon:SetTexturePixelCoordinates(unpack(GUICountDownDisplay.kPurpleIconTexCoords))
    self.nameFrame:AddChild(self.purpleIcon)
...
function GUICountDownDisplay:CreateTeamIcon()
    ...
    if PlayerUI_IsOnGreenTeam() then
    
        teamIcon:SetTexture(GUICountDownDisplay.kGreenLogoTexture)
        teamIcon:SetTexturePixelCoordinates(unpack(GUICountDownDisplay.kGreenLogoTexCoords))
        
    else
    
        teamIcon:SetTexture(GUICountDownDisplay.kPurpleLogoTexture)
        teamIcon:SetTexturePixelCoordinates(unpack(GUICountDownDisplay.kPurpleLogoTexCoords))
...
function GUICountDownDisplay:Update(deltaTime)
    ...
    local playerRecordGreen = ScoreboardUI_GetGreenScores()
    local playerRecordPurple = ScoreboardUI_GetPurpleScores()
    
    local greenNames = GetNameString(playerRecordGreen)
    local purpleNames = GetNameString(playerRecordPurple)
    
    self.greenNames:SetText(greenNames)
    self.purpleNames:SetText(purpleNames)
