...
Fade.kShadowStepCooldown = 0.67
Fade.kShadowStepJumpDelay = 0.31
Fade.kShadowStepForce = 30
Fade.kShadowStepAirForce = 15
Fade.kShadowStepDuration = 0.1
...
function Fade:TriggerShadowStep(direction)
    ...
    if canShadowStep and not self:GetIsBlinking() and not self:GetHasShadowStepCooldown() and self:GetEnergy() > kFadeShadowStepCost and not self:GetRecentlyJumped() then
        ...
        local shadowStepStrength = ConditionalValue(self:GetIsCloseToGround(1), Fade.kShadowStepForce, Fade.kShadowStepAirForce)
