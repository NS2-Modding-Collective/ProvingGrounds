...
local function InternalGeneratePath(src, dst, doSmooth, smoothDist, maxSplitPoints, allowFlying) 
    ...
    if allowFlying then
        ...
        if #points ~= 0 then
            ...
            if gDebugPathing then
                Print("using old pathing system hit entity %s", ToString(trace.entity))
            end
...
function GetRandomPointsWithinRadius(center, minRadius, maxRadius, maxHeight, numPoints, minDistance, filter)
