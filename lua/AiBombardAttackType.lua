...
function AiBombardAttackType:Init(aiEntity)
    ...
    local filter = OnlyStructuralTargetFilter(self)
    local selector = TargetSelector():Init( aiEntity, range, true, types, { filter })   
...
function OnlyStructuralTargetFilter(attackType)

    return function(target, targetPoint)
        return ( target.GetReceivesStructuralDamage and target:GetReceivesStructuralDamage(kWhipBombardDamageType) )
    end
