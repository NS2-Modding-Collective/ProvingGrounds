...
function DamageMixin:DoDamage(damage, target, point, direction, surface, altMode, showtracer)
    ...
    if attacker then
        ...
        if target and HasMixin(target, "Live") and damage > 0 then  
            ...
            damage, healthUsed = GetDamageByType(target, attacker, doer, damage, damageType, point)
            ...
            if damage > 0 then
                ...
                killedFromDamage, damageDone = target:TakeDamage(damage, attacker, doer, point, direction, healthUsed, damageType)
        ...
        if surface ~= "none" then
            ...
            elseif not surface or surface == "" then
                ...
                elseif GetAreEnemies(self, target) then
                    ...
                    if target:isa("PurpleAvatar") then
                        surface = "metal"
                    elseif target:isa("GreenAvatar") then
                        surface = "metal"
                    else
                    
                        if HasMixin(target, "Team") then
                        
                            if target:GetTeamType() == kPurpleTeamType then
                                surface = "metal"
                            else
                                surface = "metal"
                            end
