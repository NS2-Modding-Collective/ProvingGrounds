...
function GUIResourceDisplay:Update(deltaTime)
    ...
    /*
    local numSupportedHarvesters = CommanderUI_GetNumSupportedHarvesters()
    
    local color = kColorWhite    
    if numHarvesters > numSupportedHarvesters then
        color = kColorRed
    end
    
    self.towerText:SetText(ToString(numHarvesters) .. "/" .. ToString(numSupportedHarvesters))
    self.towerText:SetColor(color)
    */
