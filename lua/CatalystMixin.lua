...
function CatalystMixin:GetCanCatalyst()
    ...
    if Server then
        ...
        local canCatalyst = true
        if self.GetCanCatalystOverride then
            canCatalyst = self:GetCanCatalystOverride()
        end
        
        return not self.isCatalysted and canCatalyst and (self:isa("Embryo") or (HasMixin(self, "Research") and self:GetIsResearching()))
