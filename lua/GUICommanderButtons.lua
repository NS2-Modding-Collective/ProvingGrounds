...
function GUICommanderButtons:SharedInitializeButtons(settingsTable)
    ...
    for i = 1, settingsTable.NumberOfButtons do
        ...
        local cooldown = GUIManager:CreateGraphicItem()
        cooldown:SetSize(Vector(GUICommanderButtons.kButtonWidth, GUICommanderButtons.kButtonHeight, 0))
        cooldown:SetColor(GUICommanderButtons.kCooldownColor)
        
        buttonItem:AddChild(cooldown)
...
function GUICommanderButtons:UpdateButtonStatus(buttonIndex)
    ...
    if cooldownItem then
        ...
        if buttonCooldownFraction ~= 0 then
            cooldownItem:SetSize(Vector(GUICommanderButtons.kButtonWidth *buttonCooldownFraction, GUICommanderButtons.kButtonHeight, 0))
        end
    end
