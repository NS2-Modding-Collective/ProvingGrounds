...
function DbgTracer:Init()
    ...
    self.targetingTracer:Init("targetings", 2, { 0, 0, 1, 0.8 })
        
    self.projectileTracer = DbgProjectileTracer()
    DbgProjectileTracer:Init("projectiles", 5,{ 0, 1, 0, 0.8 }) 
    
    self.bulletHitTracer = DbgBulletTracer()
    self.bulletHitTracer:Init("bulletHits", 5, { 1, 0, 0, 0.8 }, true)
    
    self.bulletMissTracer = DbgBulletTracer()
    self.bulletMissTracer:Init("bulletMisses", 5, { 0, 1, 0, 0.8 })
    
    self.meleeHitTracer = DbgMeleeTracer()
    self.meleeHitTracer:Init("meleeHits", 5, { 1, 0, 0, 0.8 }, true)
    
    self.meleeMissTracer = DbgMeleeTracer()
    self.meleeMissTracer:Init("meleeMisses", 5, { 0, 1, 0, 0.8 })
...
function DbgBulletTracer:Trace(shooter, startPoint, trace, capsule)
    if not Shared.GetIsRunningPrediction() and self.enabled then

        local entity, bbox, bbox2, targetId, targetId2 = trace.entity, "", "", Entity.invalidId, Entity.invalidId 
        ...
        if self.enabled then
            ...
            table.insert(self.bullets, { shooter, startPoint, trace.endPoint, trace.fraction, targetId, bbox, targetId2, bbox2, capsule})  
        end
    end
end
...
function DbgBulletTracer:Flush()
    ...
    for i,data in ipairs(self.bullets) do
        local shooter, startPoint, endPoint, frac, targetId, bbox, targetId2, bbox2, capsule = unpack(data)
        local shooterName = shooter and shooter.GetName and shooter:GetName() or ToString(shooter)
