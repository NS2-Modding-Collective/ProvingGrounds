...
function Alien:UpdateCelerityEffect()
    ...
    if self.screenEffects.celerityFX then
        local celeritySpeedScalar = self.celeritySpeedScalar        
        self.screenEffects.celerityFX:SetActive(GetHasCelerityUpgrade(self))
        self.screenEffects.celerityFX:SetParameter("amount", celeritySpeedScalar / 2)
    end
...
// checking here for average extents. this should prevent most cases where you want to evolve, adjust upgrades etc. but then can't because the place is invalid
function Alien:GetHasRoomToEvolve()
    return GetHasRoomForCapsule(Vector(0.4, 0.1, 0.5), self:GetOrigin() + Vector(0, 0.1 + Embryo.kEvolveSpawnOffset, 0), CollisionRep.Default, PhysicsMask.AllButPCsAndRagdolls, self)
end
