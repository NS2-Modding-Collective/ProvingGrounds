...
local function SharedUpdate(self, deltaTime)
    ...
    if Server then
        ...
        if self.fireAttackerId ~= Entity.invalidId and self.fireDoerId ~= Entity.invalidId then
            self:DeductHealth(damageOverTime, Shared.GetEntity(self.fireAttackerId), Shared.GetEntity(self.fireDoerId))
        else
            self:SetGameEffectMask(kGameEffect.OnFire, false)
        end
