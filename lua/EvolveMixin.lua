...
local function GetNewTechIdCount(self)

    numNewTechIds = 0

    for index, techid in ipairs(self.evolvingUpgrades) do
    
        if not self:GetHasUpgrade(techid) then
            numNewTechIds = numNewTechIds + 1
        end    
    
    end
    
    return numNewTechIds

end

EvolveMixin = CreateMixin( EvolveMixin )
EvolveMixin.type = "Evolve"

EvolveMixin.kThinkTime = .1
EvolveMixin.kBaseHealth = 50

EvolveMixin.networkVars =
{
...
function EvolveMixin:SetGestationData(techIds, previousTechId, healthScalar, armorScalar)
    ...
    self.gestationTime = ConditionalValue(Shared.GetCheatsEnabled(), 2, lifeformTime + GetNewTechIdCount(self) * kUpgradeGestationTime)
    self.evolveTime = 0
...
if Server then
    ...
    function EvolveMixin:OnThink()        
        ...
        if self:GetIsAlive() and self.gestationClass ~= nil then
            ...
            local amount = GetAlienCatalystTimeAmount(EvolveMixin.kThinkTime, self)
            self.evolveTime = self.evolveTime + EvolveMixin.kThinkTime + amount
