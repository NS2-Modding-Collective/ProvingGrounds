...
{
    [kGreenTeamType] = "cinematics/vfx_materials/heal_marine.material",
    [kPurpleTeamType] = "cinematics/vfx_materials/heal_alien.material",
}

local kHealViewMaterials =
{
    [kGreenTeamType] = "cinematics/vfx_materials/heal_marine_view.material",
    [kPurpleTeamType] = "cinematics/vfx_materials/heal_alien_view.material",
}
...
function LiveMixin:GetHealthDescription()
    ...
    return string.format("Health  %s/%s  %s", ToString(math.ceil(self:GetHealth())), ToString(math.ceil(self:GetMaxHealth()))), self:GetHealthScalar()
 ...
 */
function LiveMixin:TakeDamage(damage, attacker, doer, point, direction, healthUsed, damageType, preventAlert)
    ...
    if damage > 0 then
        ...
        if self.OnTakeDamage and oldHealth ~= self.health then
    ...
    return killedFromDamage, (oldHealth - self.health)
...
function LiveMixin:DeductHealth(damage, attacker, doer, healthOnly, preventAlert)

    local healthUsed = damage
    

    local engagePoint = HasMixin(self, "Target") and self:GetEngagementPoint() or self:GetOrigin()
    self:TakeDamage(damage, attacker, doer, engagePoint, nil, healthUsed, kDamageType.Normal, preventAlert)
...
function LiveMixin:AddHealth(health, playSound, hideEffect)
...
elseif Client then
    ...
    function LiveMixin:OnPreUpdate()
        ...
        if (not self.timeLastClientUpdated or self.timeLastClientUpdated + kSynchronizeDelay < Shared.GetTime()) or GetSyncHealth(self) then
