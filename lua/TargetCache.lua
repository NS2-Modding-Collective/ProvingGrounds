    ...
    for targetId,_ in pairs(self.targetType.entityIdMap) do
        ...
        if target then
            ...
            if full or inRange then
                ...
                local visible = selector.visibilityRequired and GetCanSeeEntity(selector.attacker, target) or "N/A"
...
function TargetSelector:ValidateTarget(target)
    ...
    if target then
        ...
        if result and self.visibilityRequired then
            Server.dbgTracer.seeEntityTraceEnabled = true
            result = GetCanSeeEntity(self.attacker, target)
            Server.dbgTracer.seeEntityTraceEnabled = true
        end
    ...
    for _, target in ipairs(targets) do
        ...
        if not checkedTable[target] and prioritizer(target) then
            if self.visibilityRequired then 
                include = GetCanSeeEntity(self.attacker, target) 
            else
                include = true
            end
