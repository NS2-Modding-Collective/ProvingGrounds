...
function HiveVision_Initialize()
    ...
    HiveVision_camera:SetRenderMode( RenderCamera.RenderMode_Depth )
...
function HiveVision_SyncCamera(camera)

    HiveVision_camera:SetCoords( camera:GetCoords() )
    HiveVision_camera:SetFov( camera:GetFov() )
    HiveVision_camera:SetFarPlane( _maxDistance + 1 )
    HiveVision_screenEffect:SetParameter("time", Shared.GetTime())
    HiveVision_screenEffect:SetParameter("maxDistance", _maxDistance)
